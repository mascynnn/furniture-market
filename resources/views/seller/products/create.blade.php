<x-app-layout>
    <div class="mx-auto max-w-3xl space-y-8">
        <div>
            <p class="text-sm uppercase tracking-[0.25em] text-[#8B5E3C]">New product</p>
            <h1 class="text-3xl font-semibold text-[#333333]">Add a new handmade item</h1>
        </div>
        ...
        <div class="rounded-[28px] bg-white p-8 shadow-soft">
            <form action="{{ route('seller.products.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name') }}" class="form-control mt-2" required />
                </div>
                <div>
                    <label class="block text-sm font-medium text-[#5A4A3A]">Price</label>
                    <input type="number" name="price" value="{{ old('price') }}" class="form-control mt-2" required />
                </div>
                <div>
                    <label class="block text-sm font-medium text-[#5A4A3A]">Description</label>
                    <textarea name="description" rows="4" class="form-control mt-2">{{ old('description') }}</textarea>
                </div>
                <div class="grid gap-4 md:grid-cols-2">
                    <div>
                        <label class="block text-sm font-medium text-[#5A4A3A]">Material</label>
                        <input type="text" name="material" value="{{ old('material') }}" class="form-control mt-2" />
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-[#5A4A3A]">Size</label>
                        <input type="text" name="size" value="{{ old('size') }}" class="form-control mt-2" />
                    </div>
                </div>
                <div>
                ...
                <div class="flex items-center gap-3">
                    <input id="preorder" name="is_preorder" type="checkbox" class="h-5 w-5 rounded border-[#E9E5DD] text-wood focus:ring-wood" />
